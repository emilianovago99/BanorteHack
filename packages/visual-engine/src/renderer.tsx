...
function SurfaceView({ messages, onAction, busy }: { messages: unknown[]; onAction: (action: UIAction) => void; busy: boolean }) {
  ...
  function render(id: string): ReactNode {
    ...
    switch (node.component) {
      ...
      case 'FinancialChart': { const data = seriesSchema.parse(bound(node)); if (data.series.some(item => item.values.length !== data.labels.length)) throw new Error('Serie inválida.'); return <DatasetChart key={id} title={node.title ?? ''} type={node.chartType} {...data} />; }
