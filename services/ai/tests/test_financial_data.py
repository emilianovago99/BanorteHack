...
def test_totals_match_source_csv():
    with (DATA_DIR / "transactions.csv").open(encoding="utf-8") as file:
...
def test_filters_and_empty_period():
    data = repository().transactions(month="2026-08", merchant="Café", kind="expense")
    assert data["count"] > 0
    assert all("Café" in row["merchant"] for row in data["rows"])
    assert repository().transactions(merchant="' OR 1=1 --")["count"] == 0
    assert repository().breakdown("2030-01")["rows"] == []
...
def test_extra_debt_payment_reduces_interest_and_months():
