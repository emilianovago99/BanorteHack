...
async function start(t, options = {}) {
  const server = createApp({
    config,
    verifyAccessToken: (value) => verifyToken(value, keys, config),
    fetchAI: async () =>
      new Response(
        JSON.stringify({ balance: 123, currency: "MXN", mode: "demo" }),
      ),
    ...options,
  });
...
test("auth requerida por defecto y configuración pública sin secretos", () => {
  ...
  assert.equal(
    loadConfig({ OAUTH_ISSUER: "tenant.us.auth0.com", OAUTH_AUDIENCE: "api" })
      .issuer,
    "https://tenant.us.auth0.com/",
  );
  assert.throws(
    () => loadConfig({ OAUTH_ISSUER: "not-a-url", OAUTH_AUDIENCE: "api" }),
    (error) => !error.message.includes("not-a-url"),
  );
  assert.throws(
    () =>
      loadConfig({
...
test("rechaza tokens vencidos, otra audiencia, otro issuer y firmas falsas", async () => {
  ...
  for (const options of [
    ...
    await assert.rejects(() =>
      token(options).then((value) => verifyToken(value, keys, config)),
    );
  }
...
test("protege saldo, chat, transacciones y voz; permite config y preflight", async (t) => {
  ...
  const url = await start(t, {
    synthesize: async () => {
      synthesisCalls++;
      return Buffer.from("ID3audio");
    },
  });
  for (const [path, method] of [
    ["account", "GET"],
    ["chat", "POST"],
    ["transactions/preview", "POST"],
    ["transactions", "POST"],
    ["speech", "POST"],
  ]) {
  ...
  const invalid = await fetch(`${url}/api/account`, {
...
test("voz autenticada devuelve MP3, valida texto y limita peticiones", async (t) => {
  ...
  const url = await start(t, {
    ...
    allowSpeech: createSpeechLimiter({ limit: 1 }),
  });
  const headers = {
  ...
  const limited = await fetch(`${url}/api/speech`, {
    method: "POST",
    headers,
    body: JSON.stringify({ text: "Hola" }),
  });
...
test("modo demo nunca gasta créditos de voz", async (t) => {
  const url = await start(t, {
    ...
    synthesize: () => assert.fail("No debe llamarse"),
  });
  assert.equal(
    (
      await fetch(`${url}/api/speech`, {
...
test("adaptador ElevenLabs envía la clave solo al proveedor y devuelve audio", async () => {
  const synthesize = createSpeechService(config, {
...
test("errores de ElevenLabs no filtran el cuerpo ni la clave", async () => {
  for (const [providerStatus, status] of [
    ...
    const synthesize = createSpeechService(config, {
      fetchImpl: async () =>
        new Response("test-secret", { status: providerStatus }),
    });
    await assert.rejects(
      () => synthesize("Hola"),
      (error) =>
        error.status === status && !error.message.includes("test-secret"),
    );
  }
  await assert.rejects(
    () => createSpeechService({})("Hola"),
    (error) => error.status === 503,
  );
  await assert.rejects(
    () =>
      createSpeechService(config, {
        fetchImpl: async () => {
